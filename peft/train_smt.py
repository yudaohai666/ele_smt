...
#     # print(f"datasets_list: {datasets_list}")
#     # if data_args.dataset_name_or_path in datasets_list:
#     #     print("huggfacingface dataset")
...
def main():
    ...
    # 关闭accelerator以释放资源
    ffttrainer.accelerator.free_memory()
    ...
    original_master_addr = os.environ.get('MASTER_ADDR')
    original_master_port = os.environ.get('MASTER_PORT')
    
    # 更新训练参数以使用SMT的DeepSpeed配置
    training_args.learning_rate = smt_model_args.smt_learning_rate
    training_args.weight_decay = smt_model_args.smt_w_decay
    training_args.deepspeed = smt_deepspeed_config
    
    # 重置max steps为SMT训练所需的步数
    training_args.max_steps = -1
    
    print(f"Updated smt_model_args: {smt_model_args}")
    print(f"Updated training_args: {training_args}")
    ...
    if original_master_addr is not None:
        os.environ['MASTER_ADDR'] = original_master_addr
    if original_master_port is not None:
        os.environ['MASTER_PORT'] = original_master_port
    
    # 重新初始化分布式环境
    if original_world_size and int(original_world_size) > 1:
